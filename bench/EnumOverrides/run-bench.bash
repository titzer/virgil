#!/usr/bin/env bash
# Measure effect of RaClass elision on enums with varying override density.
# All N cases are referenced; only M have per-case method overrides.
#
...
#   M_VALUES  - space-separated M values (default: "0 1 10 50 100 N")
...
RUNS=${RUNS:-3}
M_VALUES=${M_VALUES:-"0 1 10 50 100 $N"}
TMP=/tmp/$USER/virgil-bench/enum-overrides
mkdir -p "$TMP"
...
for M in $M_VALUES; do
    SRC="$TMP/BigEnum_${N}_m${M}.v3"
    if [ ! -f "$SRC" ]; then
        bash "$DIR/generate.bash" "$N" "$M" > "$SRC"
    fi
done
...
compile_and_measure() {
    ...
    # --- runtime (best of RUNS), test 1 = val() with iterations ---
    local best_run="" run_rss=""
    if [ "$can_run" = "true" ]; then
        for run in $(seq 1 $RUNS); do
            local elapsed
            elapsed=$( { /usr/bin/time -f "%e %M" $runcmd 1 ; } 2>&1 | tail -1 )
            local secs=$(echo "$elapsed" | awk '{print $1}')
            local mem=$(echo "$elapsed" | awk '{print $2}')
    ...
    printf "  M=%-5s  compile: %6ss %6sKB   bin: %8sB   run: %6ss %6sKB\n" \
        "$label" "$best_compile" "$best_mem" "$binsize" "$best_run" "$run_rss"
}

# ---------- main ----------
echo "========================================"
echo "Enum Override Density Benchmark"
echo "  Total cases: $N   M values: $M_VALUES"
...
for target in $TARGETS; do
    echo ""
    echo "--- $target ---"
    echo "  M         compile-time  comp-RSS   binary-size      run-time  run-RSS"
    for M in $M_VALUES; do
        compile_and_measure "$M" "$TMP/BigEnum_${N}_m${M}.v3" "$target" "m${M}"
    done
done
